/**
 * Cloud Function `ingest` — единственная точка записи инвентаря в Firestore.
 *
 * Принимает от расширения JSON вида {secret, inv}, где inv — то, что собрал
 * collect.js: {user, friends, businesses[], ad_accounts[], pages[], pixels[]}.
 * Раскладывает его по таблицам базы с детерминированными id документов,
 * поэтому повторный сбор того же аккаунта ОБНОВЛЯЕТ записи, а не плодит дубли.
 *
 * Правила преобразования (порт mapping_reference.py, логика 1:1):
 *   * денежные поля Graph API приходят в ЦЕНТАХ -> делим на 100, пишем ЧИСЛОМ в долларах;
 *   * adtrust_dsl уже в долларах;
 *   * account_status / disable_reason — числовые коды -> человеческий текст;
 *   * все id пишем строкой (длинные числа теряют точность);
 *   * связи c_acc / c_bm пишем массивом ["fb_<id>"] / ["bm_<id>"] — это формат
 *     колонки типа link в приложении;
 *   * запись идёт с merge:true, поэтому авто-синк НЕ затирает поля, которые
 *     команда заполняет руками (логины, пароли, 2FA, заметки, байер и т.п.).
 */
...
// Firestore разрешает 500 операций в батче — пишем чанками с запасом.
const CHUNK = 400;

exports.ingest = onRequest(
    {cors: true, secrets: [SYNC_SECRET], region: "us-central1", maxInstances: 20},
    async (req, res) => {
      if (req.method !== "POST") return res.status(405).json({error: "POST only"});

      const body = req.body || {};
      if (!SYNC_SECRET.value() || body.secret !== SYNC_SECRET.value()) {
        logger.warn("ingest: отклонён запрос с неверным секретом");
      ...
      let records;
      try {
        records = mapInventory(inv);
      } catch (e) {
        logger.error("ingest: не смог разложить инвентарь", e);
        return res.status(400).json({error: "bad inventory"});
      }

      const stamp = {uat: NOW(), uby: "авто-синк"};
      let written = 0;
      try {
        for (let i = 0; i < records.length; i += CHUNK) {
          const batch = db.batch();
          for (const [tid, did, cells] of records.slice(i, i + CHUNK)) {
            batch.set(db.doc(`tables/${tid}/rows/${did}`), {c: cells, ...stamp}, {merge: true});
            written++;
          }
          await batch.commit();
        }
      } catch (e) {
        logger.error("ingest: запись в Firestore упала", e);
      ...
      logger.info(`ingest: ${accDoc(inv.user.id)} -> ${written} документов`, counts);
      return res.json({ok: true, written, counts});
    });

// экспорт для юнит-тестов
exports._internals = {mapInventory, money, cap, dsl, st, dis, clean, funding};
